...
if (isset($_POST["upload"])) {
    $upl = new clsadmin();
    $upl->state = $_POST["state"];
    $upl->cod=$_POST["cntry"];
    $res = $upl->state();
    if ($res == TRUE) {
        echo "<script> alert('Data Upload Successfully... ')</script>";
    } else {
        echo "<script>alert('Entered State is already exist...')</script>";
    }
}
...
if (isset($_REQUEST["delid"])) {
    $del = new clsadmin();
    //echo $_REQUEST["delid"];
    $del->state_cod = $_REQUEST["delid"];
    $res = $del->del_state();
    if ($res == TRUE) {
        echo "<script>alert ('State Delete Successfully...')</script>";
    } else {
...
if (isset($_POST["update"])) {
    ...
    $upd = new clsadmin();
    $upd->state_cod = $_POST["scod"];
    $upd->state = $_POST["sname"];
    $upd->cod=$_POST["cntry"];
    $res = $upd->update_state();
    if ($res == TRUE) {
        ...
        <div class="content-part">
            <div class="container">
                <div class="login-form">
                    <form  name="state" action="upload_state.php" method="POST">
                        <h1>Upload State</h1>
                        <hr>
                        <label for="cntry">
                            Select Country:
                        ...
                        echo'<select class="form-control" name="cntry">';
                        ...
                        <input type="text" id="state" required name="state" placeholder="Enter state name" class="form-control">
            ...
            <div class="container">
                <form name="table" action="upload_state.php" method="POST">
                    <center>
                        <table>
                            <caption><h1>Uploaded States</h1></caption>
                            <thead>
                            <th>State Code</th>
                            <th>State Name</th>
                            <th>Country Name</th>
                                ...
                                if (!isset($_REQUEST["eid"])) {
                                    ...
                                    while ($r = mysqli_fetch_row($res)) {
                                        if ($i < 11) {
                                            ...
                                            $cntrynam = $r[5];
                                            //echo $cod;
                                            echo "<tr><td>$cod</td><td>$state</td><td>$cntrynam</td><td><a href='upload_state.php?eid=$r[0]'>Edit</a> | <a href='upload_state.php?delid=$r[0]'>Delete</a> </td></tr>";
                                        }
                                        $i++;
                                    }
                                } else {
                                    if (isset($_REQUEST["eid"])) {
                                        ...
                                        $cod = $_REQUEST["eid"];
                                        $link = $con->dbconnect();
                                        $qry = "CALL edit_state($cod)";
                                        $res = mysqli_query($link, $qry);
                                        while ($r = mysqli_fetch_row($res)) {

                                            echo "<tr>";
                                            echo "<td><input readonly type='text' name='scod' value='$r[0]'> </td>";
                                            echo "<td><input type='text' name='sname' value='  $r[1]  '></td>";
                                            $obj = new clscon();
                                            $link = $obj->dbconnect();
                                            $qry = "call fetch_country()";
                                            $res = mysqli_query($link, $qry);

                                            echo'<td><select class="form-control" name="cntry">';
                                            while ($r = mysqli_fetch_row($res)) {
                                                echo '<option value=' . $r[0] . '>' . $r[2] . '</option>';
                                            }
                                            $obj->dbclose();
